...
GLuint masina1,masina2,banda,benzi_continue;
GLubyte lista_banda[13],lista_masina1[5], lista_masina2[5],lista_benziContinue[2];
static GLfloat rotTheta = 0.0;

double i = 0.0, i1 = 0.0001, j = 0.0,k=0.0,l=0.0,k1=0.0,l1=0.0,unghi1=0.0,numar=0.0,scadere=0.0;
double alpha = 1.0,beta=0.2,gamma=4,unghi=0.0001;
double xg = 0.0, yg = 0.0, r=0.6,g=0.0,b=0.0;
int p = 1,semn=1, test =0;
bool semafor=FALSE;
...
static void init(void)
{
	...
	for (k = 1; k < n; k++)
	{
		...
		//cout << hexVertex.x << "," << hexVertex.y << endl;
		glVertex2i(hexVertex.x, hexVertex.y);
	}
	hexVertex.x = 48 ;
	hexVertex.y = 40;
	...
	/*for (k = 1; k < n; k++)
	{
		hexTheta = TWO_PI *5/ 6 * k / n + TWO_PI *2/ 3;
		hexVertex.x = -76 - 182 * cos(hexTheta);
		hexVertex.y = 107 + 182 * sin(hexTheta);
		if (hexVertex.x < -120) {
			break;
		}
		if (hexVertex.x > -32) {
	...
	glClearColor(1.0, 1.0, 1.0, 1.0);
	banda = glGenLists(13);//13 benzi, 11 pe ecran //imi declar o lista in care am desenat 13 dreptunghiuri si pe care il apeles in functia de desen cu un translate
	float dist=400.0,miscare = 72;
	int contor = 0;
	for (int i = 0; i < 13; i++) {
		glNewList(banda+i, GL_COMPILE);//da imi face o lista de desene care se apeleaza impreuna
		glColor3f(0.001, 0.001, 0.001);//la masina am contur si geamuri dar ele fac parte din cealsi desen
		glBegin(GL_POLYGON);
		hexVertex.x = -15.0;
	...
	glClearColor(1.0, 1.0, 1.0, 1.0);
	benzi_continue = glGenLists(2);//2 benzi pe lateral
...
void displayHex(void)
{
	...
	glPushMatrix();
	//glTranslated(xg, yg, 0.0);
	//glTranslated(i, 0.0, 0.0);
	//glRotated(j, 0.0, 0.0, 1.0);
	//glTranslated(-xg2, -yg2, 0.0);
	...
	glListBase(benzi_continue);//asta nu stiu, stiu doar ca daca vreau sa schimb desenu dau asta listBase
	...
	/*if (test == 1){
		glPushMatrix();
		glTranslated (- 2 * xg -l, -j+k+l, 0);
		glTranslated(xg-l, yg+k+l, 0.0);
		glScaled(0.7, 0.9, 0.9);
		glRotated(unghi, 0, 0, 1);
		glTranslated(-xg-l, -yg - j-k-l, 0.0);
		glListBase(masina2);
		glCallLists(4, GL_UNSIGNED_BYTE, lista_masina2);
		glPopMatrix();
	}*/
	...
	if (test == 1){
		...
		else glTranslated((k<16)?112.0 - l:112.0- l-k, -j+k-numar , 0);
		cout << l << "," << k << endl;
		glListBase(masina1);
		glCallLists(4, GL_UNSIGNED_BYTE, lista_masina2);
		glPopMatrix();
		...
		if (l > 10) { 
			if(i1>0)glTranslated(-112.0, j - i1, 0.0);
				...
				glTranslated(-112.0, j-i1, 0.0);
			}
		}
		else glTranslated(-112.0, j , 0.0);
		//if (i1 == 0 )test = 0;
		glListBase(masina1);
		glCallLists(4, GL_UNSIGNED_BYTE, lista_masina1);
		glPopMatrix();
	}
	else {
		...
		glListBase(masina1);//nu sunt sigur, mai arta odata codu tau
		glCallLists(4, GL_UNSIGNED_BYTE, lista_masina2);
		glPopMatrix();
...
void miscas(void)
{
	...
	i = i + alpha;//face plimbarea aia fata sapte dintre masini sa dea impresia de cursa, da mai intai apas stanga dupa dreapta
	if (i <82)
		i = i + 72;
	if (j > 5)
		p=-1;
...
void mouse(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON:
		if (state == GLUT_DOWN) {//da butonu dreapt face depasirea si stang nimic,inca
			alpha = -6;
		}
		glutIdleFunc(miscas);//nu e cam la fel ca miscad doar ca nu are si indici de care aveam nevoie pt depasire
		break;
...
GLsizei winWidth = 600, winHeight = 800;
GLuint masina1,masina2,banda,benzi_continue,asfalt;
...
static void init(void)
{
	...
	g = 0.7;
	glNewList(masina1+2, GL_COMPILE);//////////////////////////////////////////////////////////Conturu masinii
	glColor3f(r, g, b);//0.6, 0.0, 0.0
	glBegin(GL_POLYGON);
	hexVertex.x = -60;
	hexVertex.y = -72;
	...
	for (int i = 0; i < 13; i++) {
		glNewList(banda+i, GL_COMPILE);//da imi face o lista de desene care se apeleaza impreuna
		glColor3f(1, 1, 1);//la masina am contur si geamuri dar ele fac parte din cealsi desen
		glBegin(GL_POLYGON);
		hexVertex.x = -15.0;
		hexVertex.y = dist - 50 - miscare*i;
		glVertex2i(hexVertex.x, hexVertex.y);
...
//void desen_masina(double r, double g, double b)
//{
//	glClearColor(0.134, 0.54, 0.134, 0.0);
//	glMatrixMode(GL_PROJECTION);
//	glOrtho(-300.0, 300.0, -400.0, 400.0, -1.0, 1.0);
...
//
//}
void interschimb_liste(GLubyte *lista_a, GLubyte *lista_b,int size) {
	GLubyte var;
	...
	if (test == 1){
		...
		if (l > 10) { 
			if(i1>0)glTranslated(-112.0, j - i1, 0.0);
				...
				glTranslated(+112.0, j , 0.0);
				//interschimb_liste(lista_masina1, lista_masina2, 2);
				//glCallLists(2, GL_UNSIGNED_BYTE, lista_masina2);
				//glTranslated(-112.0, 2*j - i1, 0.0);
			}
		}
		else glTranslated(-112.0, j , 0.0);
		//interschimb_liste(lista_masina1, lista_masina2, 2);
